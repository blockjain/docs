...
![Gateway Protocol Client (Identity)](/assets/protocol-architecture/client.png)
...
```
TODO - no longer using RSA OAEP, now using a PRE-compatible keypair baesd on elliptic curve cryptography with bilinear pairings
```
...
```
TODO -have this technically speaking for storage purposes, leave this as is for now, just removing incorrect info
```
