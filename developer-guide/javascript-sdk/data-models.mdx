...
const dataModel = await gateway.dataModel.getById(modelId);

// List Data Models created by the current user
const myModels = await gateway.dataModel.getCreatedByMe();

// Search Data Models by tags
const searchResult = await gateway.dataModel.search({
  tags: ["HR", "Employee"],
});
```
