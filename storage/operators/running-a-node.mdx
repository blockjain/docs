...
# Run the EDV node
ORG="gateway" RACK="us-west2" BINDIP="<your-ip>" PUBLICURL="<your-url>" \
docker run -d gatewaylabs/edv:testnet \
    -v=${VOLUME_LOG_LEVEL:-3} \
    edv -dataCenter=$ORG -rack=$RACK \
...
Replace `<your-ip>` with your node's public IP address and `<your-url>` with your node's public URL or IP address and port.
...
services:
  volume1:
    ...
    command: >
      -v=${VOLUME_LOG_LEVEL:-3}
      volume -dataCenter=gateway -rack=us-west2
      -dir /data
      -events.dir /events
      -port=8080 -ip=volume1 -publicUrl=${PUBLICURL}
      -mserver="testnet.edv.gateway.tech:9333"
      -preStopSeconds=1 -metricsPort=1234
    volumes:
      - volume1-data:/data
      - volume1-events:/events
...
The following environment variables customize node operation:

- `VOLUME_LOG_LEVEL`: Logging verbosity (1-5, default: 3)
- `RACK`: Rack identifier
- `ORG`: Organization identifier
- `BINDIP`: Network interface binding
