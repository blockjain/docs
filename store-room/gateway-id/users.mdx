...
In the ecosystem of the Gateway Protocol, 'users' typically refer to individual participants who interact with the system. These users form the elemental building blocks of the network, most commonly serving as the owners of Personal Data Accounts (PDAs). However, their roles can be versatile, extending to functions such as issuers and verifiers within the protocol.
...
The digital identifiers you associate with your Gateway ID function as 'reception channels' for Personal Data Accounts (PDAs). For example, if you've linked the email address sanket@mygateway.xyz to your Gateway ID, any PDA sent to that email will automatically be routed to your central Gateway ID. Refer to the diagram below for a visual representation of how a typical issuance process may unfold.

Beyond serving as PDA reception channels, these digital identifiers also streamline your access to Gateway ID. You can effortlessly authenticate yourself using any of the linked accounts, instantly granting you entry into your unified Gateway ID environment.
...
Similar to wallet-based initialization, a Gateway ID might have already been created on your behalf if an issuer sent a PDA to your email address.

You'll gain immediate access once you connect your email and find any associated PDAs waiting for you. Feel free to expand your account by adding multiple other identifiers as you see fit.
