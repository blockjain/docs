...
![Governance](assets/usecases/governance.png)
...
Sybil Resistance - Rather than the one token one vote system we see, a DID system that prioritizes the usage of data assets and a mobile app (biometric verification and user custody of keys) to authenticate humanhood ensures that we can build governance systems that are based on one person, one vote.

Weighted Systems - There is no doubt that when it comes to decision making there are active players who have a greater influence, understanding, and relationship to the problem an organization is solving. This includes people who are staking capital, people contributing/building products, active community members, and representatives with experience in a knowledge vertical. This is why Gateway's Data Asset system enables organizations to increase voting power based on the data they hold.
