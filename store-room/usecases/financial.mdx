...
![Finance](assets/usecases/finance.png)
...
The users part of Mastercard's network would be able to generate a Self-Sovereign Identity, which can connect multiple different aliases (Google Account, Wallet, Social media etc) to a single identifier. Mastercard could then define what types of Data Assets a user could obtain (KYC, Credit History, Proof of Banking etc) and who they can share it with. In this case, the applications/institutions part of Mastercard's Payments network would be able to request and verify a user's data asset to access specific products and services.
